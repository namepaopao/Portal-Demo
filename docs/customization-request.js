...
function submitRequest() {
  ...
  const history = JSON.parse(
    localStorage.getItem("lamipak-custom-requests") || "[]",
  );
  // Add status field
...
function renderHistory() {
  ...
  historyList.innerHTML = history
    .map((item) => {
      ...
      const productName =
        productTypeMap[item.productType] || item.productTypeName;
      const statusText =
        item.status === "pending"
          ? dict.status_pending || "Pending"
          : dict.status_completed || "Completed";
      const statusColor =
        item.status === "pending"
